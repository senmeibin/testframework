//定义数据一览画面JS类
SysApp.Sys.MailLogList = Class.create();

SysApp.Sys.MailLogList.prototype = Object.extend(new SysApp.Sys.CommonList(), {
    //初期化回调处理
    ...
    initTableColList: function () {
        ...
        this.colList.push(new ColInfo("应用名称", "appName", "8%", SORT_YES));
        this.colList.push(new ColInfo("邮件主题", "subject", "45%", SORT_YES));
        this.colList.push(new ColInfo("发送邮件地址", "mailFrom", "12%", SORT_YES));
        this.colList.push(new ColInfo("接受邮件地址", "mailTo", "12%", SORT_YES));
        this.colList.push(new ColInfo("重试次数", "retryCount", "5%", SORT_YES, ALIGN_CENTER));
        this.colList.push(new ColInfo("发送时间", "sendDate", "10%", SORT_YES, ALIGN_CENTER));
        this.colList.push(new ColInfo("发送结果", "sendResult", "6%", SORT_YES, ALIGN_CENTER));
    ...
    validateFormAfter: function () {
        ...
        // if (end.diff(start) > 7) {
        //     this.showErrorMessage("邮件日志最长查询周期为7天，请调整邮件发送日期的起止区间。");
        //     return false;
        // }
        return true;
    },
