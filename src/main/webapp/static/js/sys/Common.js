SysApp.Sys.Common = function () {
}

//定义数据输入画面JS类
SysApp.Sys.CommonInput = Class.create();
SysApp.Sys.CommonInput.prototype = Object.extend(new SysCmn.CoreInput(), {
...
//定义数据导入画面JS类
SysApp.Sys.CommonImport = Class.create();
SysApp.Sys.CommonImport.prototype = Object.extend(new SysCmn.CoreImport(), {});

//定义数据一览画面JS类
SysApp.Sys.CommonList = Class.create();
SysApp.Sys.CommonList.prototype = Object.extend(new SysCmn.CoreList(), {
    //取得表结构的主键名称[注意：必须实现的方法]
    getPrimaryKey: function () {
        return "uid";
    },
    ...
    initCommonCallback: function () {
        //数据同步按钮绑定事件
        this.bindEvent("btnSyncData", 'click', this.onClick_SyncData.bind(this));
    ...
    //获取API同步按钮事件
    onClick_SyncData: function () {
        this.showConfirm("确定要同步数据吗？", this.syncData.bind(this, 1));
    },
    ...
    syncData: function () {
        ...
        //Ajax处理(GET方式)
        AjaxIns.sendPOST(this.controller + "syncData", this.syncDataCallback.bind(this), params);
    },

    //同步数据回调函数
    syncDataCallback: function (response) {
