...
SysApp.Demo.CompanyInformationPopupList.prototype = Object.extend(new SysApp.Demo.CommonList(), {
    ...
    createOperationIcon: function (rowNo) {
        return "<button type='button' class='btn btn-primary' onclick='" + this.selfInstance + ".goChargesInput(" + rowNo + ");return false;'><i class='fa fa-check'></i>选择</button>";
    },

    //成本录入画面
    goChargesInput: function (rowNo) {
        var rowData = this.getRowData(rowNo);
        window.location = this.chargesInputUrl + "?companyUid=" + rowData.uid;
    },
