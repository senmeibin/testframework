//定义数据一览画面JS类
SysApp.Crm.FollowupList = Class.create();

SysApp.Crm.FollowupList.prototype = Object.extend(new SysApp.Crm.CommonList(), {
    //初期化回调处理
    ...
    fastSearch: function (status, dom) {
        ...
        if (status == 1) {
            this.setValue("followupDate$from_search", new Date().formatYMD());
            this.setValue("followupDate$to_search", new Date().formatYMD());
        }
        else if (status == 2) {
            this.setValue("followupDate$from_search", new Date().addDate("DAY", -3).formatYMD());
            this.setValue("followupDate$to_search", new Date().formatYMD());
        }
        else if (status == 3) {
            this.setValue("followupDate$from_search", new Date().addDate("DAY", -7).formatYMD());
            this.setValue("followupDate$to_search", new Date().formatYMD());
        }
        this.getList(true);
    },
    ...
    initTableColList: function () {
        ...
        this.colList.push(new ColInfo("跟进内容", "contents", "26%", SORT_YES, ALIGN_LEFT, false, CAMEL_NO));
    ...
    initAllColList: function () {
        this.allColList = [
            ...
            new ColInfo("课程顾问", "followupBelongConsultantUserName", "100px", SORT_YES, ALIGN_LEFT, false, CAMEL_NO),
            new ColInfo("跟进时间", "followupDate", "120px", SORT_YES, ALIGN_CENTER, false, CAMEL_NO),
            new ColInfo("跟进方式", "followupMethodName", "100px", SORT_YES, ALIGN_CENTER, false, CAMEL_NO),
            new ColInfo("跟进内容", "contents", "300px", SORT_YES, ALIGN_LEFT, false, CAMEL_NO),
            new ColInfo("销售进程", "saleProcessName", "100px", SORT_YES, ALIGN_CENTER, false, CAMEL_NO),
            new ColInfo("是否预约", "acceptShiftName", "100px", SORT_YES, ALIGN_CENTER, false, CAMEL_NO),
            new ColInfo("下次跟进时间", "nextFollowupDate", "120px", SORT_YES, ALIGN_CENTER, false, CAMEL_NO),
