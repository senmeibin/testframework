...
<%-- 文本框是否必须 ，默认为非必需--%>
<%@ attribute name="allowRestore" type="java.lang.String" required="false" %>
<%-- 文本框标题--%>
...
<%-- 是否能把选中的父节点的值赋值到文本框中 ， 默认父节点的值不能选中赋值--%>
<%@ attribute name="canCheckParent" type="java.lang.String" required="false" %>

<%-- ComboTree宽度--%>
<%@ attribute name="comboTreeWidth" type="java.lang.String" required="false" %>
...
<%-- 总是显示Tree --%>
<%@ attribute name="alwaysShowTree" type="java.lang.Boolean" required="false" %>

<%-- 重新加载标识 --%>
<%@ attribute name="reloadFlag" type="java.lang.Boolean" required="false" %>

<%-- 默认选择节点 --%>
<%@ attribute name="defaultSelectedNode" type="java.lang.String" required="false" %>
...
<%-- 下拉框高度--%>
<%@ attribute name="dropdownHeight" type="java.lang.String" required="false" %>
...
<div class="combo-tree-container">
    <div class='input-group combo-tree-${uniqueID}'>
        <input id="${textDomId}" style="cursor: pointer; background-color: #fff; ${empty style ? '' : style}" type="text" readonly value="" class="form-control ${empty required ? '' : required}"
    ...
    <input id="${valueDomId}DefaultSelectedNode" type="hidden" value="${defaultSelectedNode}" data-search-mode="$ignore_search" data-allow-clear="false"/>

    <input id="${valueDomId}" data-camel-field="${empty camelField ? '' : camelField}" data-alias-table="${empty aliasTable ? '' : aliasTable}" type="hidden" value="" data-allow-restore="${empty allowRestore ? 'true' : allowRestore}"
    ...
    <div id="${valueDomId}MenuContent" class="menuContent" style="display:none; position: absolute;top:31px;left:0px;z-index: 998">
        <ul id="${valueDomId}Tree" class="ztree" style="margin-top:0; width:220px; height: ${empty dropdownHeight ? '300px' : dropdownHeight };"></ul>
    </div>
</div>

<script type="text/javascript" language="javascript">
    //<![CDATA[
    function ComboCheckTree_${uniqueID}_Page_Load() {
        //解决多TAG引入的问题，全局实例变量前追加valueDomId唯一关键字
        ${jsInstance} = new SysApp.Cmn.ComboCheckTree();
        ...
        //是否能把选中的父节点的值赋值到文本框中 ， 默认父节点的值不能选中赋值
        instance.canCheckParent = "${empty canCheckParent ? "false" : canCheckParent}";
        //父DOMID
        ...
        instance.alwaysShowTree = "${alwaysShowTree}";
        instance.comboTreeWidth = "${comboTreeWidth}";
        //重新加载标识
    ...
    ComboCheckTree_${uniqueID}_Page_Load();
    //]]>
</script>