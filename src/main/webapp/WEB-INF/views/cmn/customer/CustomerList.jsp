<%@ page contentType="text/html;charset=UTF-8" pageEncoding="utf-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="shiro" uri="http://shiro.apache.org/tags" %>
<%@ taglib prefix="ctag" tagdir="/WEB-INF/tags" %>
<c:set var="ctx" value="${pageContext.request.contextPath}"/>
<script type="text/javascript" src="${staticContentsServer}/static/js/cmn/customer/CustomerList.js?${version}"></script>

<title>合同客户一览</title>
<%--注：为区分页面对象包含的数据范围，需要在MainContent前加上画面前缀--%>
<div class="wrapper CustomerList-MainContent">
    <div class="panel content-min-height">
        <div class="panel-heading">
            <em></em>
            <div class="panel-title">合同客户一览</div>
            <ctag:PageViewIcon></ctag:PageViewIcon>
        </div>
        <div class="panel-body">
            <button id="btnAdd" type="button" class="btn btn-primary">
                <i class="fa fa-plus"></i>新增合同客户
            </button>
            <button id="btnImport" type="button" class="btn btn-primary">
                <i class="fa fa-cloud-upload"></i>合同客户导入
            </button>
            <!--有配置DATA_IMPORT_CMN_API才显示同步按钮 -->
            <c:if test="${dataImportApiExist}">
                <button id="btnApiImport" type="button" class="btn btn-danger">
                    <i class="fa fa-spinner"></i>同步数据
                </button>
            </c:if>
            <div class="clear-both dashed-line">
            </div>
            <%--注：为保证客户端验证控件有效，必须设定FORM元素--%>
            <form id="MainForm">
                <div class="col-md-2 form-group">
                    <label>客户名称</label>
                    <input id="customerName" data-alias-table="main" type="text" maxlength="128" class="form-control" data-title="客户名称"/>
                </div>
                <div class="col-md-2 form-group">
                    <label>客户简称</label>
                    <input id="customerAbbreviation" data-alias-table="main" type="text" maxlength="32" class="form-control" data-title="客户简称"/>
                </div>
                <div class="col-md-2 form-group">
                    <label>主客户名称</label>
                    <input id="main_customer_name" data-alias-table="cmn_main_customer" type="text" maxlength="128" class="form-control" data-title="主客户名称"/>
                </div>
                <div class="col-md-2 form-group">
                    <label>所属行业</label>
                    <select id="industryCd" data-alias-table="main" class="form-control" data-title="所属行业">
                        <option value="">请选择</option>
                    </select>
                </div>

                <div class="clear-both dashed-line">
                </div>

                <div>
                    <button id="btnSearch" type="button" class="btn btn-primary">
                        <i class="fa fa-search"></i>查询
                    </button>
                    <shiro:hasRole name="DataExport">
                        <button id="btnExport" type="button" class="btn btn-primary">
                            <i class="fa fa-cloud-download"></i>导出
                        </button>
                    </shiro:hasRole>
                    <button id="btnClear" type="button" class="btn btn-default">
                        <i class="fa fa-eraser"></i>清空
                    </button>
                    <ctag:PagerSettingIcon pageInstance="SysApp.Cmn.CustomerListIns"/>
                </div>
            </form>
            <input type="hidden" id="jsonListData" value="${jsonDataList_Customer}"/>
            <%--上次检索条件--%>
            <input type="hidden" id="jsonSearchCondition" value="${jsonSearchCondition_Customer}"/>
            <%--下拉框选项数据集合--%>
            <input type="hidden" id="jsonOptionList" value="${jsonOptionList_Customer}"/>
            <div class="margin-top-space" id="divList" style="width: 100%">
            </div>
        </div>
    </div>
</div>

<script type="text/javascript">
    //<![CDATA[
    function CustomerList_Page_Load() {
        SysApp.Cmn.CustomerListIns = new SysApp.Cmn.CustomerList();
        var instance = SysApp.Cmn.CustomerListIns;

        instance.selfInstance = "SysApp.Cmn.CustomerListIns";
        instance.controller = "${ctx}/cmn/customer/";
        instance.inputUrl = "${ctx}/cmn/customer/input";
        instance.importUrl = "${ctx}/cmn/customer/importInit";
        instance.clientID = "CustomerList";
        instance.tableName = "cmn_customer";
        instance.detailInstance = SysApp.Cmn.CustomerDetailIns;
        instance.pageSize = "${pageSize}";
        instance.isSyncDataPage = "${isSyncDataPage}";

        instance.init();
    }

    $(function () {
        CustomerList_Page_Load();
    });
    //]]>
</script>

<%--POPUP详细控件--%>
<%@ include file="/WEB-INF/views/cmn/customer/CustomerPopupDetail.jsp" %>